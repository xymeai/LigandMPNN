...
from ligandmpnn.openfold.config import (
    NUM_EXTRA_SEQ,
    NUM_MSA_SEQ,
    NUM_RES,
    NUM_TEMPLATES,
)
from ligandmpnn.openfold.np import residue_constants as rc
...
def make_fixed_size(
    ...
    for k, v in protein.items():
        ...
        pad_size = [
            ...
            for (s1, s2) in zip(shape, schema, strict=False)
        ]
...
def random_crop_to_size(
    ...
    for k, v in protein.items():
        ...
        for i, (dim_size, dim) in enumerate(
            zip(shape_schema[k], v.shape, strict=False)
        ):
